...
def check_site():
    tools = [
        ...
        'convert-image','remove-background','ocr','pdf-to-word','edit-pdf'
    ]
    working, broken = [], []
    for tool in tools:
        try:
...
def analyze_competitors():
    ...
    my_tools = ('دمج، تقسيم، ضغط، تدوير، توقيع، حماية بكلمة مرور، علامة مائية، ختم، '
                'صورة→PDF، PDF→صورة، استخراج نص، تحويل صيغ الصور، إزالة خلفية، OCR، PDF→Word، تعديل PDF')
...
## 🧩 فجوة الأدوات
أدوات Doctor PDF الـ16 مذكورة أعلاه — اعتبرها كلها موجودة ولا تقترح بناء أي منها. قارنها بأدوات المنافسين واذكر فقط الأدوات الموجودة عندهم وغير موجودة فعلاً عند Doctor PDF؟ رتّبها حسب حجم الطلب (خاصة في السوق العربي) وحدّد لكل واحدة: هل يمكن تنفيذها داخل المتصفح بالكامل (بدون خادم)؟
...
- 16 أداة شغّالة (دمج، تقسيم، ضغط، تدوير، توقيع، حماية، علامة مائية، ختم، صورة↔PDF، استخراج نص، تحويل صور، إزالة خلفية، OCR، PDF→Word، تعديل PDF).
...
ركّز توصياتك على ما هو **غير منفّذ بعد** فقط: مثل بناء backlinks (Product Hunt / AlternativeTo / سوشيال)، روابط خارجية، تحسينات CTR إضافية، أو أداة جديدة فعلاً غير موجودة في الـ16.
...
هل توجد أداة **غير موجودة فعلاً** في قائمة الـ16 أعلاه وعليها طلب كبير (خاصة عربياً)؟ (واحدة فقط، ويجب ألا تكون من أدواتنا الحالية مثل دمج/ضغط/PDF→Word/تعديل PDF). إن لم توجد فجوة حقيقية فاكتب صراحةً: «لا توجد فجوة أدوات حرجة — ركّز على التسويق والـ SEO».
